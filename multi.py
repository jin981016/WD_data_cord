...
All = pd.read_csv(r'/home/jin/바탕화면/WD/all.txt',delim_whitespace=True,header=0)
...
def task_multirun(pp):  
    ...
    a_all_Model1_chi = np.zeros(N)
    c_T_all_Model1_chi = np.zeros(N)
    b_M_all_Model1_chi = np.zeros(N)
    resM1_chi = np.zeros((N,N))
    beta1_chi = np.zeros(N)

    c_T_all_Model1_std = np.zeros(N)
    b_M_all_Model1_std = np.zeros(N)
    a_all_Model1_std = np.zeros(N)
    ...
    for ii, t0 in tqdm(enumerate(TT)):
        ...
        linmod_Model1 = Model(linfit_Model1)
    ...
    Relation_1_std = np.array([[R_m1_std ,alpha_m1_std ,beta_m1_std , t_m1_std , res_mm1_std]])
    
    R_m1_chi= a_all_Model1_chi[kk]
    alpha_m1_chi  = b_M_all_Model1_chi[kk]
    beta_m1_chi = cc[beta0_m1_chi]
    t_m1_chi = TT[kk]
    res_mm1_chi = resM1_chi[kk,beta0_m1_chi]
    
    Relation_1_chi = np.array([[R_m1_chi ,alpha_m1_chi ,beta_m1_chi , t_m1_chi , res_mm1_chi]])
    
    np.savetxt('/home/jin/바탕화면/WD/data_chi/{}_number_data.txt'.format(pp), Relation_1_chi,delimiter=',')
    np.savetxt('/home/jin/바탕화면/WD/data_std/{}_number_data.txt'.format(pp),Relation_1_std,delimiter=',')
